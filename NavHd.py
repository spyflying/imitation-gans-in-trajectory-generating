...
def Save_nav_to_h5(array):
	...
	thisNav=f['.NAV/001']
	t0=thisNav.create_dataset("ego",data=array)
	f.close()

Nav=open("/home/sofia96/Downloads/trajactory-learning/traj1-l/2013122701001.nav","r")
...
thisTraj=Traj['.TRAJ/001']
name=""
for key in thisTraj:
	name=key
ObsCarTrajArray=np.array(thisTraj[name])
TrajTimeStamp=list(int(i) for i in list(ObsCarTrajArray[:,0]))
print(TrajTimeStamp)
num=0
EgoCarObsList=[]
...
SortedEgoCarObsList=sorted(EgoCarObsList)
